...
function updateHeroParallax() {
  ...
  heroPhoto.style.transform = `translate3d(0, ${progress * 18}px, 0) scale(1.015)`;
  heroParallax.forEach((el) => {
    const speed = Number(el.dataset.speed || 0);
    const shift = progress * speed * -150;
...
function requestHeroUpdate() {
  if (ticking) return;
  ticking = true;
  requestAnimationFrame(() => { updateHeroParallax(); ticking = false; });
}
